...
class ApacheAnalizer(DataAnalizer):
    ...
    ############ CONFIG SECTION #############
    min_for_train = 1000 # минимальное количество записей, необходимых для тренировки нс
    ...
    def store(self):
        ...
        for group_name, group_df in grouped:
            ...
            if os.path.exists(self.samples_dir + filename):
                group_df.to_csv(self.samples_dir + filename, sep='\t', index=False, mode='a', header=False)
            else:
        ...
        try:
            ...
            log('Unable save empty model', lvl='error')
    ...
    def load(self):
        ...
        self.classes_count = self.train_samples['y'].nunique()
    ...
    def prepare_train_data(self, df):
        ...
        self.classes_count = len(self.encoder_y.classes_)
    ...
    def train_model(self):
        ...
        samples_copy = self.train_samples.copy()
        ...
        self.model.add(Dense(self.classes_count, activation='softmax'))
